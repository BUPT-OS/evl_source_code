...
//dma channel
static struct dma_chan *dma_chan_tx;
static struct dma_chan *dma_chan_rx;
dma_cap_mask_t mask;
...
static void dma_complete_func_rx(void *completion)
{
    complete(completion);
...
static long user_dma_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        case USER_DMA_IOCTL_IB_TEST:            
            ...
            //prepare
            tx = dmaengine_prep_dma_memcpy(dma_chan_tx, 
...
static int user_dma_open(struct inode *inode, struct file *file)
{
    ...
    trans_config.buf_src = dma_alloc_coherent(dma_chan_tx->device->dev, BUF_LEN, &(trans_config.dma_buf_src), GFP_KERNEL);
    trans_config.buf_des = dma_alloc_coherent(dma_chan_tx->device->dev, BUF_LEN, &(trans_config.dma_buf_des), GFP_KERNEL);
...
static bool prep_dma_chans(void) 
{
    ...
    dma_chan_tx = dma_request_channel(mask, my_dma_filter, "dma-controller@16050000");
    dma_chan_rx = dma_request_channel(mask, my_dma_filter, "dma-controller@16050000");
    pr_info("end to request for dma channel\n");

    if (IS_ERR(dma_chan_tx) || IS_ERR(dma_chan_rx)) {
        pr_err("Failed to request DMA channel\n");
        return false;
    } else {
        if(dma_chan_tx && dma_chan_rx && dma_chan_tx->device && dma_chan_tx->device->dev) {
            pr_info("Got tx channel: %s\n", dma_chan_name(dma_chan_tx));
            pr_info("Got rx channel: %s\n", dma_chan_name(dma_chan_rx));
        } else {
...
{
    //release dma buffer
    dma_free_coherent(dma_chan_tx->device->dev, BUF_LEN, trans_config.buf_src, trans_config.dma_buf_src);
    dma_free_coherent(dma_chan_tx->device->dev, BUF_LEN, trans_config.buf_des, trans_config.dma_buf_des);
